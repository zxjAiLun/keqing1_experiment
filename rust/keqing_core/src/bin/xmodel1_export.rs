...
use _native::xmodel1_export::{build_xmodel1_discard_records_with_options, ExportRunOptions};
use std::env;
use std::path::PathBuf;

fn usage() -> &'static str {
    "Usage: xmodel1_export --output-dir <dir> [--data-dir <dir> ...] [--data-dirs <dir> ...] [--smoke] [--progress-every <n>] [--jobs <n>] [--limit-files <n>] [--force]"
}
...
fn run() -> Result<(), String> {
    ...
    let (file_count, manifest_path, produced_npz) =
        build_xmodel1_discard_records_with_options(&data_dirs, &output_dir_str, options)?;
    println!(
        "Rust Xmodel1 export completed: file_count={} manifest={} produced_npz={}",
        file_count, manifest_path, produced_npz
    );
    if produced_npz || options.smoke {
        return Ok(());
    }
    Err("Rust Xmodel1 preprocess completed without npz output; full export is required before training.".to_string())
}
