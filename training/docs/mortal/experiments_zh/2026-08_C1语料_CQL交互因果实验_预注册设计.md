# C1 语料 × CQL 交互因果实验：预注册设计

> **状态：`FROZEN v1`；`training_not_authorized`；`evaluation_not_authorized`。**
>
> candidate experiment ID：`C1_corpus_cql_interaction_2026_08`。
> 本文件是冻结后的 scientific contract，不是 training recipe，不创建 registry entry，
> 也不授权训练、生成、evaluation 或 checkpoint 产出。freeze 后不得根据实验结果
> 修改统计规则、intervention、seed、训练长度或 evaluation scale。
...
| evaluation status | `evaluation_not_authorized` |
...
没有发现 `C1_corpus_cql_interaction_2026_08` 冲突。本冻结 contract 只允许使用该实验 ID，
...
本 frozen preregistration 的来源仓库 baseline 是
`8c2c2eeb530bc54515e063be915292644a5163ee`，freeze 开始前已核验
`HEAD = origin/main`。工作树另有一个未跟踪的 `1.md`，不属于本实验文件，保持
untouched / untracked。
...
“CQL=0 是否更强”。唯一 frozen intervention 是 continuation 阶段把 CQL contribution
weight 设为零。
...
每个新 cell 使用 training seeds `20260806`、`20260807`、`20260808`。冻结后的唯一
解析状态为：

```text
historical CURRENT training reuse = APPROVED

new training cells:
  M0 × CQL_OFF × seeds 20260806/07/08
  D1 × CQL_OFF × seeds 20260806/07/08

new continuation trainings = exactly 6
```

上述内容是 frozen design，不是 training authorization。
...
除 corpus 和 CQL intervention 外，frozen contract 固定如下：
...
| archive | steps 固定为 `70001, 70010, 70100, 70500, 71000, 72000` |
...
因此本 frozen contract 在后续 registry、implementation/preflight 和
authorization-only ordering 中，所需的新 continuation trainings 固定为：
...
本 freeze 不创建 registry entry、不授权 implementation、training 或 evaluation，
也没有启动任何训练。
...
```text
70k                = K0 parent，SHA 6c0e70058644e02671440ddf7dd2b41c637ae7c2132c9154595593ab690d49e0
ext_mortal         = authoritative external model，SHA 0a88ddad649804d085491b5397d895f596b0e55f30632c549ea145bb44786563
...
```text
70k                = 同一 K0 parent，SHA 6c0e70058644e02671440ddf7dd2b41c637ae7c2132c9154595593ab690d49e0
ext_mortal         = 同一 authoritative external model，SHA 0a88ddad649804d085491b5397d895f596b0e55f30632c549ea145bb44786563
...
evaluation seed blocks 固定为：

```text
training seed 20260806: 1700000..1700999
training seed 20260807: 1710000..1710999
training seed 20260808: 1720000..1720999
```

使用上述冻结的 seed blocks、`seed_key=8192`、random-seat、B250 shard、rank points、
fresh evaluator source 和 opponent population。CURRENT/OFF 使用完全相同的 evaluation
...
### 8.3 Historical runtime reference boundary
...
因此 C1 固定采用统一 fresh Linux evaluation；不保留 Windows runtime 复用分支。
historical current checkpoint 是否进入新 CURRENT lineup，只取决于第 7 节已通过的
training reuse gate；historical current raw logs 永不进入 C1 primary statistic。

### 8.4 Frozen fresh Linux/CUDA evaluation provenance

C1 fresh evaluation 的 evaluator、native runtime、Mortal revision、模型和 device
contract 在 freeze 时写死如下：

```text
evaluator repository    = zxjAiLun/keqing1_experiment
evaluator source commit = 8c2c2eeb530bc54515e063be915292644a5163ee
evaluator source path   = training/mortal/four_player_native.py
evaluator content SHA256= 6663badce02e04150f3ccc13f0ab3d4568e3743ed556ed22c9b3dd23cbf775fe
evaluator Git blob OID  = 90487ea578850966f9c33bbcfb5e75a0fc589081

Mortal source revision  = 813859fc8110ea178f56f009994bc4f1b9fee645
Mortal content status   = no content diff from revision; local differences are file-mode metadata only
native file             = /media/bailan/DISK/AUbuntuProject/project/keqing1_experiment/.venv/lib/python3.12/site-packages/riichi.so
native SHA256           = da687ececbae8c803c99fe58fb8f66d0e4b9e762eb2bb7257a2115c57e5dd82b

python                  = 3.12.13
torch                   = 2.13.0+cu130
CUDA runtime            = 13.0
device                  = NVIDIA GeForce RTX 4060 Laptop GPU
device contract         = cuda required; AMP=false
native batch            = 250 hanchans
games per shard         = 250
shards per seed         = 4
seat mode               = random
seed key                = 8192
...
K0 evaluation checkpoint path          = /media/bailan/DISK/AUbuntuProject/keqing-data/mortal/authoritative/D3_top2_discard_v1_2026_08/models/K0_70k/mortal_default_70k_promoted_candidate.pth
K0 evaluation checkpoint SHA256        = 6c0e70058644e02671440ddf7dd2b41c637ae7c2132c9154595593ab690d49e0
ext_mortal checkpoint path             = /media/bailan/DISK/AUbuntuProject/keqing-data/mortal/authoritative/D3_top2_discard_v1_2026_08/models/ext_mortal/external_mortal_20240308_best_min.pth
ext_mortal checkpoint SHA256           = 0a88ddad649804d085491b5397d895f596b0e55f30632c549ea145bb44786563
```

其中 `M0_CURRENT`、`D1_CURRENT` 的六个 checkpoint SHA 见第 7.2 节；CQL_OFF 的
config/checkpoint 在 freeze 时尚未创建，未来 implementation/preflight 只能绑定与本
文件完全一致的 semantic contract。历史 protocol 中的 evaluator commit
`feb2ad675a7576b437c674b4e65e9735df16a83e` 只作为 historical reference，不是 C1
fresh evaluation source。
...
## 10. Frozen machine adjudication

本节是本 frozen preregistration 的唯一 machine adjudication rule。artifact gate、
runtime gate、pairing gate、bootstrap `B`、bootstrap seed 和失败标签均在本文件中
冻结；freeze 后不得根据中间结果修改。
...
相减。实现沿用 Mortal corrected summarizer 的 complete-hanchan unit 和
...
power claim；本 frozen contract 将其固定为 design limitation，不得依据中间结果扩大
到 6 seeds、增加 hanchans 或改变 primary gate。
...
`interaction_not_confirmed` 只表示本设计下没有通过预先冻结的 interaction gate，
...
## 13. Freeze 后 implementation 前的最小 gate 清单

以下是 freeze 后 review 和 implementation/preflight 阶段的 checklist，不是本 freeze
commit 的执行命令：

- 完成 frozen preregistration review 后，单独注册 registry entry；本 freeze commit 不注册。
- implementation/preflight bind CQL_OFF config 与 config SHA 到本文件的 frozen semantic
  contract，并验证 objective-side diff；不允许新增其它 scientific cell、intervention 或
  CQL weight。
- 复核并使用第 7.3 节已冻结的 loader compatibility audit：`COMPLETE / PASS`；historical
  CURRENT training reuse：`APPROVED`；new CQL_OFF trainings：`exactly 6`；不得替换或
  重新解释该 gate。
...
- implementation bind 新 CQL_OFF training artifact root、seed、parent SHA、optimizer state
  digest 和 `70000 -> 72000` completion proof，并逐项验证与本 preregistration 一致。
- implementation/preflight bind 并验证第 8.4 节已冻结的统一 fresh Linux evaluation
  runtime、native SHA、Mortal revision、evaluator source、seed blocks、B250 shards、
  opponent model SHA 和 rank points。
- 对 CURRENT/OFF 两个独立四人 lineup 使用相同 hanchan identities；不得把历史 evaluation
  logs 混入 C1 primary statistic。
- 实现并验证已经冻结的 complete-hanchan interaction row schema、`bootstrap B=5000`、
  `bootstrap seed=20260818` 和 adjudication rule。
- 完成 implementation/preflight review 后，才提交 authorization-only training commit。
- authorization-only commit 永远发生在 prereg freeze 与 registry registration 之后；本 frozen
  preregistration 本身永不触发 optimizer step。

## 14. Frozen 交付声明

```text
EXPERIMENT ID = C1_corpus_cql_interaction_2026_08
PREREG STATUS = FROZEN v1
...
本 freeze commit 只冻结并改名 scientific contract：

```text
training/docs/mortal/experiments_zh/2026-08_C1语料_CQL交互因果实验_预注册设计.md
```

既有 feasibility artifact 仍为非-authoritative root，loader report SHA 固定为：
...
sha256 = aff26cee9fa32781b426b3f02302df0e0625baee9bdd5be9c000fce2acbff46c
```

`research_registry.json`、frozen K0 prereg、D1/D2/D3 artifacts 和 authoritative output
root 均未修改；本 freeze 没有启动训练、生成、evaluation、optimizer 或 checkpoint。

## 15. Freeze 后治理顺序

```text
C1 preregistration FREEZE
  ↓
web review of frozen contract
  ↓
独立 registry registration
  ↓
implementation / config generation
  ↓
implementation + preflight review
  ↓
authorization-only training commit
  ↓
six CQL_OFF continuation trainings
  ↓
completion review
  ↓
fresh 6000-hanchan evaluation authorization/run
  ↓
locked C1 adjudication
  ↓
closure
```

training authorization 与 evaluation authorization 可以分阶段治理；freeze commit 本身
永远不授权 optimizer step、训练、生成或 evaluator run。

## 16. Registry boundary

本 freeze commit 明确不修改 `training/docs/mortal/research_registry.json`。下一笔独立
registration commit 必须引用本 frozen commit 和 frozen preregistration 文件 SHA256；在
registration 前不得创建 C1 training config、formal training output root 或 authorization-only
training commit。
