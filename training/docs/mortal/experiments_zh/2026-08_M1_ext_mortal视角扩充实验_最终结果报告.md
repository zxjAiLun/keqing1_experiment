...
* **Positive Seeds**: **`1/3`** (Seed 20260807 为正)
* **95% 分层置信区间**: **`[-9.375, +3.435375] pt`**
* **预注册门禁核验**:
  * $3/3$ Seed 全为正向: `False` (1/3 正向)
...
* **Positive Seeds**: **`2/3`** (Seeds 20260807, 20260808 为正)
* **95% 分层置信区间**: **`[-4.515375, +6.465375] pt`**
* **预注册门禁核验**:
  * $3/3$ Seed 全为正向: `False` (2/3 正向)
  * CI 下界大于 0: `False`
...
1. **实验假设未获支持 (promotion not supported)**：在当前 Mortal DQN + CQL 离线强化学习框架下，同分布扩充 6,000 局 `ext_mortal` 视角语料（6k → 12k 半庄），**没有带来稳定、可晋级的收益证据**（M1 vs M0 均值为 `-2.970 pt`，1/3 正向；M1 vs K0 均值为 `+0.990 pt`，2/3 正向；两项检验的 95% CI 均跨零）。
2. **结果性质**：全链路数据完整性检查、Checkpoints 权重有限性校验、3,000 局对局日志与统计分析均严格通过；结果为 mixed/null，未通过预注册 promotion gate。
3. **晋级裁决**：
   * **Recipe 晋级**: **`False`**
   * **Checkpoint 晋级**: **`False`**
