...
ENTRIES: list[tuple[str, str, str, str, bool, str]] = [
    ...
    # ---- P4-M11: direct PG continuation to the K0-replacement gate -----------
    ("P4-M11", "dir", f"{SP}/P4-M11_direct_pg_C4_plus32",
     "★ P4-M11 单一线系 hard50k → C4 → U32（32 cycle × 256 on-policy 半庄）；此条目只计顶层 *.pth", False, "*.pth"),
    ("P4-M11", "file", f"{SP}/P4-M11_direct_pg_C4_plus32/RESULT.md",
     "★ P4-M11 运行目录内结果副本（两门结论、优势来源、可用模型、复现入口）", False, ""),
    ("P4-M11", "file", f"{SP}/P4-M11_direct_pg_C4_plus32/p4m11_result.json",
     "★ 32 cycle 运行级结果（完成态、最终 Adam step、评测端点）", False, ""),
    ("P4-M11", "file", f"{SP}/P4-M11_direct_pg_C4_plus32/cycles.jsonl",
     "逐 cycle 记录（采集/更新/Adam step/checkpoint 身份）", False, ""),
    ("P4-M11", "file", f"{SP}/P4-M11_direct_pg_C4_plus32/U32.pth",
     "训练端点（含 optimizer_state/cycle，130 MB）；**不可直接加载评测**，评测请用 U32_eval_weights.pth", False, ""),
    ("P4-M11", "file", f"{SP}/P4-M11_direct_pg_C4_plus32/U32_eval_weights.pth",
     "★ 被评测的候选权重（U32.pth 去掉 optimizer/cycle 的纯重封装，650+2 张量逐位一致）", True, ""),
    ("P4-M11", "file", f"{SP}/P4-M11_direct_pg_C4_plus32/U32_eval_weights.provenance.json",
     "★ 上述导出的来源与校验记录（源 sha256、张量一致性、arena loader 接受）", True, ""),
    ("P4-M11", "dir", f"{EV}/ovt_p4m11_gateA_solo",
     "★ Gate A 方向 1：U32 单挑 vs 3×K0_70k（256 seeds × 4 座位；含 metrics/run_identity/detailed_stats）", False, "logs/*.json.gz"),
    ("P4-M11", "dir", f"{EV}/ovt_p4m11_gateA_mirror",
     "★ Gate A 方向 2：K0_70k 单挑 vs 3×U32", False, "logs/*.json.gz"),
    ("P4-M11", "dir", f"{EV}/ovt_p4m11_gateB_solo",
     "★ Gate B 方向 1：U32 单挑 vs 3×ext_mortal", False, "logs/*.json.gz"),
    ("P4-M11", "dir", f"{EV}/ovt_p4m11_gateB_mirror",
     "★ Gate B 方向 2：ext_mortal 单挑 vs 3×U32", False, "logs/*.json.gz"),
    ("P4-M11", "file", f"{EV}/p4m11_gateA_verdict.json",
     "★ Gate A 裁决（practical_gate = supported，G = +2.6367 [+0.2930, +4.9294]）", True, ""),
    ("P4-M11", "file", f"{EV}/p4m11_gateB_verdict.json",
     "★ Gate B 裁决（eve_gate = not_supported，G = -5.0317 [-7.5515, -2.4976]）", True, ""),
    ("P4-M11", "file", f"{EV}/p4m11_gate_stats_zh.md",
     "★ 中文双向详细统计摘要（四个方向、逐座比率、双方样本口径）", True, ""),
    ("P4-M11", "file", f"{EV}/p4m11_gate_stats_zh.json",
     "同上，机器可读版", True, ""),
    ("P4-M11", "file", "training/docs/mortal/2026-09/2026-09-14_P4-M11_结果总览与交付索引.md",
     "★ P4-M11 **唯一结果入口**（训练/两门/详细统计/模型来源+sha/复现/目录约定）", True, ""),
    ("P4-M11", "dir", f"{EV}/_p4m11_gateA_logs",
     "临时目录（前缀 _）：启动脚本、原始日志、失败启动与自检输出；正式脚本已移入 training/mortal/", False, "*"),
]
