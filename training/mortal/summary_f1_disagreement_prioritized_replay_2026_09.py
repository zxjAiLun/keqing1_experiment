"""Statistical summary and crossed bootstrap adjudication for F1 prioritized replay experiment."""
...
logger = logging.getLogger("f1_summary")
...
def _scores_and_ranks_from_events(events: list[dict[str, Any]], log_name: str) -> tuple[list[float], list[int]]:
    """Reconstruct final scores with exact reach_accepted semantics, then ranks (R1/R2/S1 parity)."""
...
def _verify_eval_manifest(
    ...
    *,
    ...
    """Fail-closed validation of the eval manifest and its SHA chain bindings (R1/R2/S1 parity)."""
    ...
    for seed_val in TRAINING_SEEDS:
        ...
        for cond in ("r2_control", "priority_variant"):
...
def adjudicate_f1_prioritized_replay(
    training_dir: Path = F1_TRAINING_DIR,
    eval_dir: Path = F1_EVAL_DIR,
    summary_dir: Path = F1_SUMMARY_DIR,
    ...
    tr_man_path = training_dir / "f1_training_manifest.json"
    ev_man_path = eval_dir / "f1_eval_manifest.json"
    ...
    # Shape: (3, 1000) for Primary (PriorityVariant - R2 Control) and Absolute (PriorityVariant - K0)
    ...
    for s_idx, t_seed in enumerate(target_seeds):
        ...
        for log_path in all_logs:
            ...
            seat_var = n2s[f"F1_PriorityVariant_seed_{t_seed}"]
    ...
    verdict, recipe_promo, ckpt_promo, k1_target = adjudicate_f1_verdict(
    ...
    # Selection statistics aggregate from the verified training manifest.
    selection_stats_by_seed: dict[str, Any] = {}
    for s in target_seeds:
        seg = tr_man["row_identity"]["by_seed"][f"seed_{s}"]["selection_stats"]
        selection_stats_by_seed[f"seed_{s}"] = {
            "high_signal_count_min": int(seg["high_signal_count_min"]),
            "high_signal_count_max": int(seg["high_signal_count_max"]),
            "high_signal_rate_mean": float(seg["high_signal_rate_mean"]),
            "priority_duplicate_rate_mean": float(seg["priority_duplicate_rate_mean"]),
        }
    ...
    summary = {
        ...
        "selection_stats": selection_stats_by_seed,
        "metrics": {
            "total_games": total_logs_verified,
            "primary_contrast_priority_variant_minus_r2_control": {
            ...
            "absolute_contrast_priority_variant_minus_k0": {
    ...
    summary_path = summary_dir / "f1_summary.json"
...
def main() -> None:
    ...
    parser.add_argument("--training-dir", type=Path, default=F1_TRAINING_DIR)
    parser.add_argument("--eval-dir", type=Path, default=F1_EVAL_DIR)
    parser.add_argument("--summary-dir", type=Path, default=F1_SUMMARY_DIR)
    args = parser.parse_args()

    logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
    res = adjudicate_f1_prioritized_replay(training_dir=args.training_dir, eval_dir=args.eval_dir, summary_dir=args.summary_dir)
