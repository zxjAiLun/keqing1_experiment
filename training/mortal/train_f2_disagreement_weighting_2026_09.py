"""Training runner for F2 disagreement-weighting-without-resampling pilot (variant-only)."""
...
logger = logging.getLogger("f2_training")
...
def compute_weighted_f2_losses(
    *,
    q_out: torch.Tensor,
    masks: torch.Tensor,
    actions: torch.Tensor,
    ...
    """F2 weighted objective: weighted_mean over per-row value/CQL/next-rank losses.
...
def train_f2_weighted_variant(
    seed: int,
    device: str = "cuda",
    output_dir: Path = F2_TRAINING_DIR,
) -> tuple[Path, str, str, dict[str, Any]]:
    """Train exactly 400 steps of the F2 weighted variant on one seed.

    Every base row is used exactly once per batch; only the loss weighting
    changes. Returns (checkpoint_path, checkpoint_sha, base_row_digest, weighting_stats).
    """
    ...
    for step_idx in range(1, OPTIMIZER_STEPS + 1):
        ...
        row_weights_np = compute_f2_row_weights(disagreement)
        ...
        if weights_unique not in ([AGREEMENT_WEIGHT], [DISAGREEMENT_WEIGHT], [AGREEMENT_WEIGHT, DISAGREEMENT_WEIGHT]):
            raise ContractError(f"Illegal weight values at step {step_idx}: {weights_unique}")
        ...
        losses = compute_weighted_f2_losses(
            q_out=q_out,
            masks=masks,
            actions=actions,
        ...
        if step_idx % 100 == 0 or step_idx == OPTIMIZER_STEPS:
            logger.info(
                "[Seed %d | weighted_variant] Step %d/%d (step %d) | total: %.4f | value: %.4f | cql: %.4f | disagree: %d (%.1f%%)",
    ...
    logger.info("[Seed %d | weighted_variant] Completed 400 optimizer steps in %.2f seconds", seed, elapsed)

    checkpoint_name = f"mortal_weighted_variant_70400_seed_{seed}.pth"
    ...
    return checkpoint_path, ckpt_sha, base_digest.hexdigest(), weighting_stats
...
def run_f2_training(
    ...
    """Execute variant-only 400-step training for all 3 seeds under F2 protocol."""
    ...
    for s in target_seeds:
        ...
        logger.info(
            "================ F2 Seed %d (WeightedVariant: rows once, disagreement weight 2.0) ================",
            s,
        )
        var_path, var_sha, var_base_digest, weighting_stats = train_f2_weighted_variant(
        ...
        row_usage_ok = all(rec["rows_used"] == BATCH_SIZE for rec in weighting_stats["per_step"])
        all_row_usage_ok = all_row_usage_ok and row_usage_ok
        weights_ok = all(
            rec["weights_unique"] in ([AGREEMENT_WEIGHT], [DISAGREEMENT_WEIGHT], [AGREEMENT_WEIGHT, DISAGREEMENT_WEIGHT])
            for rec in weighting_stats["per_step"]
        )
    ...
    hard_gates: dict[str, bool] = {
        ...
        "all_3_variant_checkpoints_saved": all(
            Path(data["weighted_variant"]["path"]).exists() for data in checkpoints_manifest.values()
        ),
        "all_seeds_base_row_identity_matches_r2_control": all_rows_match_r2,
        "no_resampling_row_usage_exact": all_row_usage_ok,
        "weights_2_1_normalized_all_batches": all_weights_ok,
    ...
    manifest_path = output_dir / "f2_training_manifest.json"
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=F2_TRAINING_DIR)
    args = parser.parse_args()

    logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
    res = run_f2_training(device=args.device, output_dir=args.output_dir)
