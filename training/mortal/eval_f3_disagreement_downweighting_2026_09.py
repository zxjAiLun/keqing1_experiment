"""Evaluation runner for F3 disagreement-downweighting pilot: 3 panels x 1000 hanchans."""
...
logger = logging.getLogger("f3_eval")
...
def run_single_shard(
    ...
    cmd = [
        ...
        f"--model=F3_DownweightedVariant_seed_{seed_key}={var_path}",
...
def run_f3_evaluation(
    ...
    for training_seed in target_seeds:
        ...
        var_path = Path(seed_data["downweighted_variant"]["path"])
        if not var_path.exists():
            raise FileNotFoundError(f"F3 DownweightedVariant checkpoint for seed {training_seed} not found at {var_path}")
        var_sha = sha256_file(var_path)
        if seed_data["downweighted_variant"]["sha256"] != var_sha:
            raise ContractError(f"DownweightedVariant checkpoint SHA mismatch for seed {training_seed}")
        ...
        panels_manifest[panel_key] = {
            ...
            "models": {
                ...
                "downweighted_variant": {"name": var_path.name, "path": str(var_path), "sha256": var_sha},
            },
        }
    ...
    manifest_path = eval_dir / "f3_eval_manifest.json"
...
def main() -> None:
    ...
    res = run_f3_evaluation(training_dir=args.training_dir, eval_dir=args.eval_dir, device=args.device)
