"""Statistical summary and crossed bootstrap adjudication for S1 score_to_go auxiliary experiment."""
...
logger = logging.getLogger("s1_summary")
...
def _scores_and_ranks_from_events(events: list[dict[str, Any]], log_name: str) -> tuple[list[float], list[int]]:
    """Reconstruct final scores with exact reach_accepted semantics, then ranks (R1/R2 parity)."""
...
def _verify_eval_manifest(
    ...
    *,
    ...
    """Fail-closed validation of the eval manifest and its SHA chain bindings (R1/R2 parity)."""
    ...
    for seed_val in TRAINING_SEEDS:
        ...
        for cond in ("r2_control", "aux_variant"):
...
def adjudicate_s1_auxiliary(
    training_dir: Path = S1_TRAINING_DIR,
    eval_dir: Path = S1_EVAL_DIR,
    summary_dir: Path = S1_SUMMARY_DIR,
    ...
    tr_man_path = training_dir / "s1_training_manifest.json"
    ev_man_path = eval_dir / "s1_eval_manifest.json"
    ...
    # Shape: (3, 1000) for Primary (AuxVariant − R2 Control) and Absolute (AuxVariant − K0)
    ...
    for s_idx, t_seed in enumerate(target_seeds):
        ...
        for log_path in all_logs:
            ...
            seat_var = n2s[f"S1_AuxVariant_seed_{t_seed}"]
    ...
    verdict, recipe_promo, ckpt_promo, k1_target = adjudicate_s1_verdict(
    ...
    summary = {
        ...
        "metrics": {
            ...
            "primary_contrast_aux_variant_minus_r2_control": {
            ...
            "absolute_contrast_aux_variant_minus_k0": {
    ...
    summary_path = summary_dir / "s1_summary.json"
...
def main() -> None:
    ...
    parser.add_argument("--training-dir", type=Path, default=S1_TRAINING_DIR)
    parser.add_argument("--eval-dir", type=Path, default=S1_EVAL_DIR)
    parser.add_argument("--summary-dir", type=Path, default=S1_SUMMARY_DIR)
    args = parser.parse_args()

    logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
    res = adjudicate_s1_auxiliary(training_dir=args.training_dir, eval_dir=args.eval_dir, summary_dir=args.summary_dir)
