"""Frozen contract, paths, and schemas for F2 disagreement-weighting-without-resampling pilot."""
...
EXPERIMENT_ID = "F2_disagreement_weighting_without_resampling_pilot_2026_09"
F2_ROOT = REPO_ROOT / "artifacts" / "experiments" / EXPERIMENT_ID
F2_TRAINING_DIR = F2_ROOT / "training"
F2_EVAL_DIR = F2_ROOT / "evaluation"
F2_SUMMARY_DIR = F2_ROOT / "summary"

TRAINING_MANIFEST_SCHEMA = "keqing.mortal.f2_training_manifest.v1"
EVAL_MANIFEST_SCHEMA = "keqing.mortal.f2_eval_manifest.v1"
SUMMARY_SCHEMA = "keqing.mortal.f2_summary.v1"
...
# F2 weighting protocol: no resampling, no dropping, no duplication.
...
class ContractError(RuntimeError):
    """Raised when any F2 contract invariant is breached."""
...
def compute_f2_row_weights(disagreement: np.ndarray) -> np.ndarray:
    """Row weights: 2.0 for disagreement rows, 1.0 otherwise.
...
def validate_f2_seed_set(seeds: list[int] | None) -> list[int]:
    """Fail-closed: F2 runners accept only the exact frozen seed set."""
    if seeds is None:
        return list(TRAINING_SEEDS)
    if list(seeds) != list(TRAINING_SEEDS):
        raise ContractError(
            f"F2 requires the exact frozen seed set {TRAINING_SEEDS}; got {seeds}"
        )
    return list(seeds)
...
def verify_training_manifest(tr_man: dict) -> bool:
    """Fail-closed validation of an F2 training manifest."""
    ...
    wgt = tr_man.get("disagreement_weighting")
    if not isinstance(wgt, dict):
        raise ContractError(f"Training manifest disagreement_weighting missing or invalid: {wgt}")
    ...
    if wgt.get("resampling") is not False:
        raise ContractError("disagreement_weighting resampling must be false")
    ...
    if wgt.get("q_margin_threshold") is not None:
        raise ContractError("disagreement_weighting must not set a Q-margin threshold")
    if wgt.get("uses_reward_or_eval_signal") is not False:
        raise ContractError("disagreement_weighting must not use reward or evaluation signals")
    ...
    for s in TRAINING_SEEDS:
        ...
        var = entry.get("weighted_variant")
        if not isinstance(var, dict):
            raise ContractError(f"Training manifest missing weighted_variant for {key}")
        if not var.get("sha256") or len(var.get("sha256", "")) != 64:
            raise ContractError(f"weighted_variant sha missing/invalid for {key}")
        if not var.get("path"):
            raise ContractError(f"weighted_variant path missing for {key}")
    ...
    for s in TRAINING_SEEDS:
        ...
        stats = seg.get("weighting_stats")
        if not isinstance(stats, dict):
            raise ContractError(f"Training weighting_stats missing for {ks}")
        if int(stats.get("batches", -1)) != OPTIMIZER_STEPS:
            raise ContractError(f"Training weighting_stats batches mismatch for {ks}")
        ...
        for rec in per_step:
            ...
            if sorted(rec.get("weights_unique", [])) not in ([AGREEMENT_WEIGHT, DISAGREEMENT_WEIGHT], [AGREEMENT_WEIGHT], [DISAGREEMENT_WEIGHT]):
                raise ContractError(f"Training per-step weights_unique mismatch in {ks}: {rec.get('weights_unique')}")
...
def adjudicate_f2_verdict(
    primary_seed_means: list[float],
    primary_ci_lower: float,
    absolute_seed_means: list[float],
    absolute_ci_lower: float,
) -> tuple[str, bool, bool, str | None]:
    """Adjudicate the three-state F2 pilot verdict.

    F2 is an adaptive pilot: no verdict directly promotes K1, so checkpoint
    promotion and k1 are always false/null. `reweighting_promising` /
