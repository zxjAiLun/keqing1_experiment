"""Evaluation runner for S1 score_to_go auxiliary experiment: 3 panels x 1000 hanchans."""
...
logger = logging.getLogger("s1_eval")
...
def run_single_shard(
    ...
    cmd = [
        ...
        f"--model=S1_AuxVariant_seed_{seed_key}={var_path}",
...
def run_s1_evaluation(
    ...
    for training_seed in target_seeds:
        ...
        var_path = Path(seed_data["aux_variant"]["path"])
        if not var_path.exists():
            raise FileNotFoundError(f"S1 AuxVariant checkpoint for seed {training_seed} not found at {var_path}")
        var_sha = sha256_file(var_path)
        if seed_data["aux_variant"]["sha256"] != var_sha:
            raise ContractError(f"AuxVariant checkpoint SHA mismatch for seed {training_seed}")
        ...
        panels_manifest[panel_key] = {
            ...
            "models": {
                ...
                "aux_variant": {"name": var_path.name, "path": str(var_path), "sha256": var_sha},
            },
        }
    ...
    manifest_path = eval_dir / "s1_eval_manifest.json"
...
def main() -> None:
    ...
    res = run_s1_evaluation(training_dir=args.training_dir, eval_dir=args.eval_dir, device=args.device)
