"""Preflight and final-call guards for D3 continuation shards (1800250..1805999)."""
...
from training.mortal.d3_production_contract import (
    ...
    load_authoritative_smoke_protocol,
    mortal_lineage,
    parse_model_specs,
    project_lineage,
    sha256_file,
...
def implementation_manifest_continuation(repo_root: Path = REPO_ROOT) -> dict[str, dict[str, str]]:
    repo_root = repo_root.resolve()
    manifest: dict[str, dict[str, str]] = {}
    for relative in CONTINUATION_IMPLEMENTATION_PATHS:
        path = repo_root / relative
        if not path.is_file():
            raise ContractError(f"continuation implementation file missing: {path}")
...
def build_preflight(args: argparse.Namespace) -> dict[str, Any]:
    ...
    seed_start = shard_seed_start(args.shard_index)
    seed_end = shard_seed_end_exclusive(args.shard_index)
    errors = [
        *project["errors"],
        *lineage["errors"],
        *native["errors"],
    ...
    preflight = {
        "schema": "keqing.mortal.d3_continuation_shard_preflight.v1",
        ...
        "production_implementation": implementation_manifest_continuation(REPO_ROOT),
        ...
        "execution_shape": {
            ...
            "auto_continue_remaining": 23 * GAMES_PER_SHARD,
...
def final_call_guard(args: argparse.Namespace, preflight: dict[str, Any]) -> dict[str, Any]:
    ...
    lineage = continuation_lineage(REPO_ROOT)
    native = mortal_lineage(args.mortal_root.resolve())
    errors = [*project["errors"], *lineage["errors"], *native["errors"]]
    ...
    current_implementation = implementation_manifest_continuation(REPO_ROOT)
    if current_implementation != preflight["production_implementation"]:
        errors.append("continuation implementation changed after initial preflight")
