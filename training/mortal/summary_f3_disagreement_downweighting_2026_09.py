"""Statistical summary and crossed bootstrap adjudication for F3 disagreement-downweighting pilot."""
...
logger = logging.getLogger("f3_summary")
...
def _verify_eval_manifest(
    ...
    *,
    ...
    for seed_val in TRAINING_SEEDS:
        ...
        for cond in ("r2_control", "downweighted_variant"):
...
def adjudicate_f3_downweighting(
    training_dir: Path = F3_TRAINING_DIR,
    eval_dir: Path = F3_EVAL_DIR,
    summary_dir: Path = F3_SUMMARY_DIR,
    ...
    tr_man_path = training_dir / "f3_training_manifest.json"
    ev_man_path = eval_dir / "f3_eval_manifest.json"
    ...
    for s_idx, t_seed in enumerate(target_seeds):
        ...
        for log_path in all_logs:
            ...
            seat_var = n2s[f"F3_DownweightedVariant_seed_{t_seed}"]
    ...
    verdict, recipe_promo, ckpt_promo, k1_target = adjudicate_f3_verdict(
    ...
    downweighting_stats_by_seed: dict[str, Any] = {}
    for s in target_seeds:
        seg = tr_man["row_identity"]["by_seed"][f"seed_{s}"]["downweighting_stats"]
        downweighting_stats_by_seed[f"seed_{s}"] = {
    ...
    summary = {
        ...
        "downweighting_stats": downweighting_stats_by_seed,
        "metrics": {
            "total_games": total_logs_verified,
            "primary_contrast_downweighted_variant_minus_r2_control": {
            ...
            "absolute_contrast_downweighted_variant_minus_k0": {
    ...
    summary_path = summary_dir / "f3_summary.json"
...
def main() -> None:
    ...
    parser.add_argument("--training-dir", type=Path, default=F3_TRAINING_DIR)
    parser.add_argument("--eval-dir", type=Path, default=F3_EVAL_DIR)
    parser.add_argument("--summary-dir", type=Path, default=F3_SUMMARY_DIR)
    args = parser.parse_args()

    logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
    res = adjudicate_f3_downweighting(training_dir=args.training_dir, eval_dir=args.eval_dir, summary_dir=args.summary_dir)
