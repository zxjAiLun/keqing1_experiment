"""Frozen contract, paths, and schemas for F3 disagreement-downweighting pilot."""
...
EXPERIMENT_ID = "F3_disagreement_downweighting_pilot_2026_09"
F3_ROOT = REPO_ROOT / "artifacts" / "experiments" / EXPERIMENT_ID
F3_TRAINING_DIR = F3_ROOT / "training"
F3_EVAL_DIR = F3_ROOT / "evaluation"
F3_SUMMARY_DIR = F3_ROOT / "summary"

TRAINING_MANIFEST_SCHEMA = "keqing.mortal.f3_training_manifest.v1"
EVAL_MANIFEST_SCHEMA = "keqing.mortal.f3_eval_manifest.v1"
SUMMARY_SCHEMA = "keqing.mortal.f3_summary.v1"
...
# F3 downweighting protocol: no resampling, no dropping, no duplication.
...
class ContractError(RuntimeError):
    """Raised when any F3 contract invariant is breached."""
...
def compute_f3_row_weights(disagreement: np.ndarray) -> np.ndarray:
    """Row weights: 0.5 for disagreement rows, 1.0 otherwise.

    No resampling, no dropping, no duplication: the output has the same length
    as the input and every base row is used exactly once.
    """
    mask = np.asarray(disagreement, dtype=bool)
    return np.where(mask, DISAGREEMENT_WEIGHT, AGREEMENT_WEIGHT).astype(np.float64)


def validate_f3_weight_values(weights_unique: list[float]) -> None:
    """Fail-closed: per-step observed weight values must be a subset of {0.5, 1.0}.

    Canonical validation shared by the per-batch runtime gate, the per-seed
    aggregation gate, and the manifest verifier. Order-insensitive (set
    semantics) because weights_unique is produced by sorted() ascending.
    """
    observed = set(float(w) for w in weights_unique)
    allowed = {AGREEMENT_WEIGHT, DISAGREEMENT_WEIGHT}
    if not observed or not observed.issubset(allowed):
        raise ContractError(f"Illegal weight values: {sorted(observed)}; allowed {sorted(allowed)}")


def validate_f3_seed_set(seeds: list[int] | None) -> list[int]:
    """Fail-closed: F3 runners accept only the exact frozen seed set."""
    if seeds is None:
        return list(TRAINING_SEEDS)
    if list(seeds) != list(TRAINING_SEEDS):
        raise ContractError(
            f"F3 requires the exact frozen seed set {TRAINING_SEEDS}; got {seeds}"
        )
    return list(seeds)
...
def verify_training_manifest(tr_man: dict) -> bool:
    """Fail-closed validation of an F3 training manifest."""
    ...
    wgt = tr_man.get("disagreement_downweighting")
    if not isinstance(wgt, dict):
        raise ContractError(f"Training manifest disagreement_downweighting missing or invalid: {wgt}")
    if wgt.get("disagreement_weight") != DISAGREEMENT_WEIGHT:
        raise ContractError(f"disagreement_weight mismatch: {wgt.get('disagreement_weight')}")
    if wgt.get("agreement_weight") != AGREEMENT_WEIGHT:
        raise ContractError(f"agreement_weight mismatch: {wgt.get('agreement_weight')}")
    if wgt.get("resampling") is not False:
        raise ContractError("disagreement_downweighting resampling must be false")
    if wgt.get("row_usage") != "each_row_exactly_once":
        raise ContractError(f"row_usage mismatch: {wgt.get('row_usage')}")
    if wgt.get("normalization") != "batch_weight_sum":
        raise ContractError(f"normalization mismatch: {wgt.get('normalization')}")
    if wgt.get("applied_to") != ["value_loss_row", "cql_loss_row", "next_rank_loss_row"]:
        raise ContractError(f"applied_to mismatch: {wgt.get('applied_to')}")
    if wgt.get("q_margin_threshold") is not None:
        raise ContractError("disagreement_downweighting must not set a Q-margin threshold")
    if wgt.get("uses_reward_or_eval_signal") is not False:
        raise ContractError("disagreement_downweighting must not use reward or evaluation signals")
    ...
    for s in TRAINING_SEEDS:
        ...
        var = entry.get("downweighted_variant")
        if not isinstance(var, dict):
            raise ContractError(f"Training manifest missing downweighted_variant for {key}")
        if not var.get("sha256") or len(var.get("sha256", "")) != 64:
            raise ContractError(f"downweighted_variant sha missing/invalid for {key}")
        if not var.get("path"):
            raise ContractError(f"downweighted_variant path missing for {key}")
    ...
    for s in TRAINING_SEEDS:
        ...
        stats = seg.get("downweighting_stats")
        if not isinstance(stats, dict):
            raise ContractError(f"Training downweighting_stats missing for {ks}")
        if int(stats.get("batches", -1)) != OPTIMIZER_STEPS:
            raise ContractError(f"Training downweighting_stats batches mismatch for {ks}")
        per_step = stats.get("per_step")
        if not isinstance(per_step, list) or len(per_step) != OPTIMIZER_STEPS:
            raise ContractError(f"Training per-step weighting log missing/incomplete for {ks}")
        for rec in per_step:
            if int(rec.get("rows_used", -1)) != BATCH_SIZE:
                raise ContractError(f"Training per-step rows_used mismatch in {ks}: {rec.get('rows_used')}")
            if int(rec.get("disagreement_count", -1)) < 0 or int(rec.get("disagreement_count", -1)) > BATCH_SIZE:
                raise ContractError(f"Training per-step disagreement_count out of range in {ks}")
            try:
                validate_f3_weight_values(rec.get("weights_unique", []))
            except ContractError:
                raise ContractError(f"Training per-step weights_unique mismatch in {ks}: {rec.get('weights_unique')}") from None
            if abs(float(rec.get("weight_sum", -1)) - (rec["disagreement_count"] * DISAGREEMENT_WEIGHT + (BATCH_SIZE - rec["disagreement_count"]) * AGREEMENT_WEIGHT)) > 1e-9:
                raise ContractError(f"Training per-step weight_sum inconsistent in {ks}")
...
def adjudicate_f3_verdict(
    primary_seed_means: list[float],
    primary_ci_lower: float,
    absolute_seed_means: list[float],
    absolute_ci_lower: float,
) -> tuple[str, bool, bool, str | None]:
    """Adjudicate the three-state F3 pilot verdict.

    F3 is an adaptive pilot: no verdict directly promotes K1, so checkpoint
    promotion and k1 are always false/null. `downweighting_promising` /
    `control_recovery_only` identify a follow-up confirmation candidate only.
    """
    primary_pass = (
    ...
    if primary_pass and absolute_pass:
        verdict = "downweighting_promising"
    elif primary_pass and not absolute_pass:
        verdict = "control_recovery_only"
    else:
