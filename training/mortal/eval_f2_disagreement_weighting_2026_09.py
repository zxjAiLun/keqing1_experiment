"""Evaluation runner for F2 disagreement-weighting pilot: 3 panels x 1000 hanchans."""
...
def run_single_shard(
    ...
    cmd = [
        ...
        f"--model=F2_WeightedVariant_seed_{seed_key}={var_path}",
...
def run_f2_evaluation(
    ...
    for training_seed in target_seeds:
        ...
        var_path = Path(seed_data["weighted_variant"]["path"])
        if not var_path.exists():
            raise FileNotFoundError(f"F2 WeightedVariant checkpoint for seed {training_seed} not found at {var_path}")
        var_sha = sha256_file(var_path)
        if seed_data["weighted_variant"]["sha256"] != var_sha:
            raise ContractError(f"WeightedVariant checkpoint SHA mismatch for seed {training_seed}")
        ...
        panels_manifest[panel_key] = {
            ...
            "models": {
                ...
                "weighted_variant": {"name": var_path.name, "path": str(var_path), "sha256": var_sha},
            },
        }
    ...
    manifest_path = eval_dir / "f2_eval_manifest.json"
...
def main() -> None:
    ...
    res = run_f2_evaluation(training_dir=args.training_dir, eval_dir=args.eval_dir, device=args.device)
