"""Evaluation runner for F1 behavior-disagreement prioritized replay experiment: 3 panels x 1000 hanchans."""
...
logger = logging.getLogger("f1_eval")
...
def run_single_shard(
    ...
    cmd = [
        ...
        f"--model=F1_PriorityVariant_seed_{seed_key}={var_path}",
...
def run_f1_evaluation(
    ...
    for training_seed in target_seeds:
        ...
        var_path = Path(seed_data["priority_variant"]["path"])
        if not var_path.exists():
            raise FileNotFoundError(f"F1 PriorityVariant checkpoint for seed {training_seed} not found at {var_path}")
        var_sha = sha256_file(var_path)
        if seed_data["priority_variant"]["sha256"] != var_sha:
            raise ContractError(f"PriorityVariant checkpoint SHA mismatch for seed {training_seed}")
        ...
        panels_manifest[panel_key] = {
            ...
            "models": {
                ...
                "priority_variant": {"name": var_path.name, "path": str(var_path), "sha256": var_sha},
            },
        }
    ...
    manifest_path = eval_dir / "f1_eval_manifest.json"
...
def main() -> None:
    ...
    res = run_f1_evaluation(training_dir=args.training_dir, eval_dir=args.eval_dir, device=args.device)
