"""Statistical summary and crossed-bootstrap adjudication for the T1 K0-anchor pilot."""
...
logger = logging.getLogger("t1_summary")
...
def _verify_eval_manifest(
    ...
    *,
    ...
    for seed_val in TRAINING_SEEDS:
        ...
        for cond in ("r2_control", "anchor_variant"):
...
def adjudicate_t1_policy_anchor(
    training_dir: Path = T1_TRAINING_DIR,
    eval_dir: Path = T1_EVAL_DIR,
    summary_dir: Path = T1_SUMMARY_DIR,
    ...
    ev_man_path = eval_dir / "t1_eval_manifest.json"
    ...
    for s_idx, t_seed in enumerate(target_seeds):
        ...
        for log_path in all_logs:
            ...
            seat_var = n2s[f"T1_AnchorVariant_seed_{t_seed}"]
    ...
    mechanism_audit = tr_man["mechanism_audit"]
    mechanism_pass = bool(mechanism_audit["all_seed_directions_pass"])
    verdict, recipe_promo, ckpt_promo, k1_target = adjudicate_t1_verdict(
    ...
    anchor_stats_by_seed: dict[str, Any] = {}
    for s in target_seeds:
        seg = tr_man["row_identity"]["by_seed"][f"seed_{s}"]["anchor_stats"]
        anchor_stats_by_seed[f"seed_{s}"] = {
            "anchor_kl_first": float(seg["anchor_kl_first"]),
            "anchor_kl_last": float(seg["anchor_kl_last"]),
            "anchor_kl_mean": float(seg["anchor_kl_mean"]),
            "elapsed_seconds": float(seg["elapsed_seconds"]),
        }
    ...
    hard_gates: dict[str, bool] = {
        ...
        "mechanism_audit_evaluated": isinstance(mechanism_audit.get("by_seed"), dict),
    }
    ...
    summary = {
        ...
        "calibration": tr_man["calibration"],
        "policy_anchor": tr_man["policy_anchor"],
        "anchor_training_stats": anchor_stats_by_seed,
        "mechanism_audit": mechanism_audit,
        "mechanism_pass": mechanism_pass,
        "metrics": {
            "total_games": total_logs_verified,
            "primary_contrast_anchor_variant_minus_r2_control": {
            ...
            "absolute_contrast_anchor_variant_minus_k0": {
    ...
    summary_path = summary_dir / "t1_summary.json"
...
def main() -> None:
    ...
    parser.add_argument("--summary-dir", type=Path, default=T1_SUMMARY_DIR)
    args = parser.parse_args()

    logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
    res = adjudicate_t1_policy_anchor(training_dir=args.training_dir, eval_dir=args.eval_dir, summary_dir=args.summary_dir)
