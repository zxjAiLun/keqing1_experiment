"""Statistical summary and crossed bootstrap adjudication for F2 disagreement-weighting pilot."""
...
from training.mortal.f2_disagreement_weighting_contract_2026_09 import (
    ...
    sha256_file,
    validate_f2_seed_set,
    verify_training_manifest,
)

logger = logging.getLogger("f2_summary")
...
def _scores_and_ranks_from_events(events: list[dict[str, Any]], log_name: str) -> tuple[list[float], list[int]]:
    """Reconstruct final scores with exact reach_accepted semantics, then ranks (R1/R2/S1/F1 parity)."""
...
def _verify_eval_manifest(
    ...
    *,
    ...
    """Fail-closed validation of the eval manifest and its SHA chain bindings (R1/R2/S1/F1 parity)."""
    ...
    for seed_val in TRAINING_SEEDS:
        ...
        for cond in ("r2_control", "weighted_variant"):
...
def adjudicate_f2_weighting(
    training_dir: Path = F2_TRAINING_DIR,
    eval_dir: Path = F2_EVAL_DIR,
    summary_dir: Path = F2_SUMMARY_DIR,
    seeds: list[int] | None = None,
) -> dict[str, Any]:
    """Load manifests, verify all 3000 logs across 3 panels, run crossed bootstrap, and adjudicate."""
    target_seeds = validate_f2_seed_set(seeds)
    check_directory_empty_or_nonexistent(summary_dir)
    summary_dir.mkdir(parents=True, exist_ok=True)

    tr_man_path = training_dir / "f2_training_manifest.json"
    ev_man_path = eval_dir / "f2_eval_manifest.json"
    ...
    # Verify all checkpoints manifest <-> disk <-> eval SHA (strict three-way binding).
    for s in target_seeds:
        ...
        for cond in ("r2_control", "weighted_variant"):
            ...
            if not path_str or not sha_rec or len(sha_rec) != 64:
                raise ContractError(f"Training manifest checkpoint sha/path missing/invalid for {key}/{cond}")
            p = Path(path_str)
            if not p.exists():
                raise FileNotFoundError(f"Checkpoint not found at: {path_str}")
            ...
            if ev_sha != sha_rec or ev_sha != disk_sha:
                raise ContractError(f"Three-way checkpoint SHA mismatch for {key}/{cond}")

    # Shape: (3, 1000) for Primary (WeightedVariant - R2 Control) and Absolute (WeightedVariant - K0)
    ...
    for s_idx, t_seed in enumerate(target_seeds):
        ...
        lineup = _lineup_for_seed(t_seed)
        ...
        for log_path in all_logs:
            ...
            seat_k0 = n2s["K0_70k"]
            seat_ctrl = n2s[f"R2_Control_seed_{t_seed}"]
            seat_var = n2s[f"F2_WeightedVariant_seed_{t_seed}"]
            ...
            if col in col_to_vals or game_id in seen_ids:
        ...
        if sorted(game_ids_this_panel) != expected_ids:
            per_panel_ok = False
            raise ContractError(
                f"Panel {panel_dir.name} game ID verification failed: expected contiguous range {EVAL_SEED_START}..{EVAL_SEED_END_EXCLUSIVE - 1}"
            )
    ...
    crossed_ok = bool(
        np.isfinite(grand_primary_mean)
        and np.isfinite(primary_ci).all()
        and np.isfinite(grand_absolute_mean)
        and np.isfinite(absolute_ci).all()
    )

    verdict, recipe_promo, ckpt_promo, k1_target = adjudicate_f2_verdict(
    ...
    # Weighting statistics aggregate from the verified training manifest.
    weighting_stats_by_seed: dict[str, Any] = {}
    for s in target_seeds:
        seg = tr_man["row_identity"]["by_seed"][f"seed_{s}"]["weighting_stats"]
        weighting_stats_by_seed[f"seed_{s}"] = {
            "disagreement_count_min": int(seg["disagreement_count_min"]),
            "disagreement_count_max": int(seg["disagreement_count_max"]),
            "disagreement_rate_mean": float(seg["disagreement_rate_mean"]),
        }
    ...
    summary = {
        ...
        "pilot_policy": "adaptive pilot: no verdict directly promotes K1",
        "hard_gates": hard_gates,
        "weighting_stats": weighting_stats_by_seed,
        "metrics": {
            "total_games": total_logs_verified,
            "primary_contrast_weighted_variant_minus_r2_control": {
            ...
            "absolute_contrast_weighted_variant_minus_k0": {
    ...
    summary_path = summary_dir / "f2_summary.json"
...
def main() -> None:
    ...
    parser.add_argument("--training-dir", type=Path, default=F2_TRAINING_DIR)
    parser.add_argument("--eval-dir", type=Path, default=F2_EVAL_DIR)
    parser.add_argument("--summary-dir", type=Path, default=F2_SUMMARY_DIR)
    args = parser.parse_args()

    logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
    res = adjudicate_f2_weighting(training_dir=args.training_dir, eval_dir=args.eval_dir, summary_dir=args.summary_dir)
