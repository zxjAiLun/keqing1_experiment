#!/usr/bin/env python3
"""Prepare the pre-registered legal-mean value objective A/B."""
...
EXPERIMENT_ID = "legal_mean_value_ab_2026_07"
...
def prepare_config(base: dict[str, Any], run_dir: Path, mode: str) -> dict[str, Any]:
...
def main() -> None:
    ...
    output_root = (args.output_root / EXPERIMENT_ID).resolve()
    base = load_config(source_config)
    parent_sha = sha256_file(parent)
    groups = (
        ("C_behavior_action_mc", "behavior_action_mc"),
        ("V_legal_mean_mc", "legal_mean_mc"),
    )
    runs: list[dict[str, Any]] = []
    for group_id, objective_mode in groups:
        for seed in seeds:
            run_dir = output_root / group_id / f"seed_{seed}"
            config_path = run_dir / "config.toml"
            labels_path = run_dir / "ext_mortal_train_labels.txt"
            config = prepare_config(base, run_dir, objective_mode)
            command = [
                "uv", "run", "--no-sync", "python", "scripts/run_mortal_dqn_offline.py",
                "--config", str(config_path),
                "--target-steps", str(args.target_steps),
                "--device", "cuda",
                "--num-workers", "0",
                "--seed", str(seed),
                "--data-seed", str(seed),
                "--initialize-from", str(parent),
                "--initialize-optimizer-from", str(parent),
                "--initial-steps", str(args.initial_steps),
                "--archive-steps", ",".join(str(step) for step in ARCHIVE_STEPS),
                "--archive-dir", str((run_dir / "checkpoints").resolve()),
                "--log-every", "50",
            ]
            runs.append(
                {
    ...
    manifest = {
        "schema": "keqing.mortal.legal_mean_value_ab.v1",
        "experiment_id": EXPERIMENT_ID,
        "hypothesis": "move final-rank MC supervision from behavior action Q to mean legal Q",
        "parent_checkpoint": str(parent),
        "parent_sha256": parent_sha,
    ...
    if manifest["git_dirty"] and not args.dry_run:
        raise SystemExit("working tree is dirty; commit the objective implementation before training")
